...
const Navbar = () => {
  ...
  const burgerStyle = "burger-item bg-black w-8 h-1 mb-1 bg-light ease-in-out duration-150 ";
  ...
  return (
    ...
    >
      ...
      <div
        className={
          "nav-menus duration-300 bg-white bg-opacity-90 md:bg-opacity-0 absolute top-0 py-20 md:static  md:min-h-fit min-h-screen left-0 md:w-auto w-full flex items-center px-5 justify-center " + (open ? "-top-0% " : "-top-[2000%] ")
        }
      >
        <ul className="menu-list flex flex-col md:flex-row gap-y-12 justify-center items-center md:gap-16">
          {menus.map((menu, i) => (
            <li key={i} className="hover:text-[#fbe9d0] text-xl md:text-lg" onClick={closeMenu}>
      ...
      <div id="navigation-burger" onClick={() => setOpen(!open)} className="cursor-pointer md:hidden z-10">
